...
public class ColourGrid extends Component {
	private static Color[] colours = { Color.BLACK, Color.GRAY, Color.LIGHT_GRAY, Color.WHITE, Color.RED, Color.ORANGE,
			Color.YELLOW, Color.GREEN, new Color(0, 150, 0), new Color(0, 255, 255), new Color(0, 148, 255), Color.BLUE,
			new Color(72, 0, 255), Color.MAGENTA, new Color(255, 0, 110) };
	...
	public ColourGrid(int left, int top, int width, Canvas canvas, Slider redSlider, Slider greenSlider,
			Slider blueSlider) {
		super(left, top);
		this.width = width;
		this.canvas = canvas;
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive,
		...
		if (GuiHelper.isMouseInside(mouseX, mouseY, xPosition + 1, yPosition + 1, endX - 2, endY - 2)) {
			...
			drawRect(xPosition + (boxX * 10) + 1, yPosition + (boxY * 10) + 1, xPosition + (boxX * 10) + 11,
					yPosition + (boxY * 10) + 11, hoverColour);
		}
	}
