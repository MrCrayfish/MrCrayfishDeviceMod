...
public class Game extends Component {
	...
	public Game(int left, int top, int mapWidth, int mapHeight) throws Exception {
		...
		tiles = new Tile[4][this.mapWidth * this.mapHeight];
	}
	...
	public boolean loadMap(int width, int height, int[][] data) {
		...
		mapWidth = width;
		mapHeight = height;
		tiles = new Tile[4][mapWidth * mapHeight];

		for (int layer = 0; layer < data.length; layer++) {
			for (int tile = 0; tile < data[0].length; tile++) {
				tiles[layer][tile] = registeredTiles.get(data[layer][tile]);
			}
		}
		return true;
	}
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive,
		...
		if (editorMode) {
			drawRect(xPosition - 1, yPosition - 1, xPosition + (mapWidth * Tile.WIDTH) + 1,
					yPosition + (mapHeight * Tile.HEIGHT) + 1, Color.DARK_GRAY.getRGB());
		}
		...
		// System.out.println("Rendered game in " + (System.currentTimeMillis() -
		// start));
	}
	...
	public void nextLayer() {
		...
		if (next <= (Layer.values().length - 1)) {
			currentLayer = Layer.values()[next];
		}
		System.out.println(currentLayer.name());
	}

	public void prevLayer() {
		int prev = currentLayer.layer - 1;
		if (prev >= 0) {
			currentLayer = Layer.values()[prev];
		}
		System.out.println(currentLayer.name());
	}
