...
public class TileBlock extends Tile {
	...
	public void render(Game game, int x, int y, Layer layer) {
		if (layer == Layer.BACKGROUND) {
			...
			if (!game.isFullTile(layer, x, y + 1) && (this != Tile.water)) {
				...
				RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH),
						game.yPosition + (y * Tile.HEIGHT) + 6, layer.zLevel, this.x * 16, this.y * 16, WIDTH, 2, 16,
						4);
		...
		if ((game.getTile(layer.up(), x, y - 1) != this) || (layer == Layer.FOREGROUND)) {
			RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH), (game.yPosition + (y * Tile.HEIGHT)) - 6,
					layer.zLevel, topX * 16, topY * 16, WIDTH, HEIGHT, 16, 16);
		}

		GL11.glColor4f(0.6F, 0.6F, 0.6F, 1F);
		RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH), game.yPosition + (y * Tile.HEIGHT),
				layer.zLevel, this.x * 16, this.y * 16, WIDTH, 6, 16, 16);
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
	}
	...
	public void renderForeground(Game game, int x, int y, Layer layer) {
		...
		game.getTile(layer, x, y + 1);
		if (game.getTile(layer, x, y + 1) == Tile.water) {
			GL11.glColor4f(0.6F, 0.6F, 0.6F, 1F);
			RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH), game.yPosition + (y * Tile.HEIGHT) + 6,
					layer.zLevel, this.x * 16, this.y * 16, WIDTH, 1, 16, 2);
			GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		}
	}
}
