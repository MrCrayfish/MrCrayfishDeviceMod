...
public class TileGrass extends Tile {
	...
	public void render(Game game, int x, int y, Layer layer) {
		...
		if (!game.isFullTile(layer, x, y + 1)) {
			...
			RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH), game.yPosition + (y * Tile.HEIGHT) + 6,
					Tile.dirt.x * 16, Tile.dirt.y * 16, Tile.WIDTH, Tile.HEIGHT, 16, 16);
	...
	public void renderForeground(Game game, int x, int y, Layer layer) {
		...
		if (game.getTile(layer, x, y - 1) == Tile.water) {
			RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH), (game.yPosition + (y * Tile.HEIGHT)) - 1,
					16, 16, 8, 1, 16, 2);
		}
		...
		if (game.getTile(layer, x - 1, y) == Tile.water) {
			RenderUtil.drawRectWithTexture((game.xPosition + (x * Tile.WIDTH)) - 1, game.yPosition + (y * Tile.HEIGHT),
					0, 16, 1, 6, 2, 12);
		}

		if (game.getTile(layer, x + 1, y) == Tile.water) {
			RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH) + 8, game.yPosition + (y * Tile.HEIGHT), 2,
					16, 1, 6, 2, 12);
		}
	}
