...
public class TileEnchantmentTable extends Tile {
	...
	public void render(Game game, int x, int y, Layer layer) {
		if ((game.getTile(layer.up(), x, y - 1) != this) || (layer == Layer.FOREGROUND)) {
			RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH), (game.yPosition + (y * Tile.HEIGHT)) - 4,
					layer.zLevel, (topX * 16) + 16, topY * 16, WIDTH, HEIGHT, 16, 16);
		}

		GL11.glColor4f(0.6F, 0.6F, 0.6F, 1F);
		RenderUtil.drawRectWithTexture(game.xPosition + (x * Tile.WIDTH), game.yPosition + (y * Tile.HEIGHT) + 2,
				layer.zLevel, this.x * 16, (this.y * 16) + 4, WIDTH, 4, 16, 12);
