...
public class TileGrid extends Component {
	...
	public void init(Layout layout) {
		...
		btnNextCategory.setClickListener((c, mouseButton) -> {
		...
		btnPrevCategory.setClickListener((c, mouseButton) -> {
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive,
		...
		for (int i = 0; i < tabTiles.size(); i++) {
			...
			if (GuiHelper.isMouseInside(mouseX, mouseY, tileX - 1, tileY - 1, tileX + 12, tileY + 12)
					|| (game.getCurrentTile() == tile)) {
