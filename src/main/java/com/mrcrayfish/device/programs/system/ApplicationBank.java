...
public class ApplicationBank extends SystemApplication {
	...
	public void init() {
		...
		textWelcome = new Text(TextFormatting.BLACK + "Hello " + Minecraft.getMinecraft().player.getName()
				+ ", welcome to The Emerald Bank! How can I help you?", 62, 25, 125);
		layoutStart.addComponent(textWelcome);

		btnDepositWithdraw = new Button(54, 74, "View Account");
		btnDepositWithdraw.setSize(76, 20);
		btnDepositWithdraw.setClickListener((c, mouseButton) -> {
	...
	public static class TaskWithdraw extends Task {
		...
		public void processRequest(NBTTagCompound nbt, World world, EntityPlayer player) {
			...
			if (account.withdraw(amount)) {
				...
				if (remaining > 0) {
					world.spawnEntity(new EntityItem(world, player.posX, player.posY, player.posZ,
							new ItemStack(Items.EMERALD, remaining)));
				}
