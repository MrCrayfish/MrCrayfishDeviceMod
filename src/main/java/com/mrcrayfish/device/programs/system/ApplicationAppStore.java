...
public class ApplicationAppStore extends Application {
	...
	public void init() {
		...
		setCurrentLayout(layoutHome);

		/*
		 * appsLabel = new Label("Application List", 5, 5);
		 * super.addComponent(appsLabel);
		 * 
		 * apps = new ItemList<AppInfo>(5, 18, 100, 6); apps.addItem(new
		 * AppInfo("Blah")); apps.addItem(new AppInfo("Blah")); apps.addItem(new
		 * AppInfo("Blah")); apps.setListItemRenderer(new ListItemRenderer<AppInfo>(20)
		 * {
		 * 
		 * @Override public void render(AppInfo e, Gui gui, Minecraft mc, int x, int y,
		 * int width, int height, boolean selected) { if(selected) gui.drawRect(x, y, x
		 * + width, y + height, Color.DARK_GRAY.getRGB()); else gui.drawRect(x, y, x +
		 * width, y + height, Color.GRAY.getRGB()); e.renderIcon(mc, x + 3, y + 3);
		 * gui.drawString(mc.fontRendererObj, e.getFormattedId(), x + 20, y + 6,
		 * Color.WHITE.getRGB());
		 * 
