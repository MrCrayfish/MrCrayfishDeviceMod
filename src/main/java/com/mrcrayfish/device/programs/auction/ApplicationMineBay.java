...
public class ApplicationMineBay extends Application {
	...
	private String[] categories = { "Building", "Combat", "Tools", "Food", "Materials", "Redstone", "Alchemy", "Rare",
			"Misc" };
	...
	public void init() {
		...
		labelBalance.setAlignment(Component.ALIGN_RIGHT);
		super.addComponent(labelBalance);

		final Label labelMoney = new Label("$0", 295, 13);
		labelMoney.setAlignment(Component.ALIGN_RIGHT);
		...
		items.setListItemRenderer(new ListItemRenderer<AuctionItem>(20) {
			@Override
			public void render(AuctionItem e, Gui gui, Minecraft mc, int x, int y, int width, int height,
					boolean selected) {
				if (selected) {
				...
				GlStateManager.pushMatrix();
				{
					...
					mc.fontRenderer.drawString(TimeUtil.getTotalRealTime(e.getTimeLeft()), 0, 11,
							Color.LIGHT_GRAY.getRGB(), false);
				}
				GlStateManager.popMatrix();

				String price = "$" + e.getPrice();
				mc.fontRenderer.drawString(price, ((x - mc.fontRenderer.getStringWidth(price)) + width) - 5, y + 6,
						Color.YELLOW.getRGB());
			}
		});
		...
		inventory.setClickListener((c, mouseButton) -> {
			if (inventory.getSelectedSlotIndex() != -1) {
				ItemStack stack = Minecraft.getMinecraft().player.inventory
						.getStackInSlot(inventory.getSelectedSlotIndex());
				if (!stack.isEmpty()) {
		...
		buttonAddCancel.setClickListener((c, mouseButton) -> restoreDefaultLayout());
		...
		buttonAddNext.setEnabled(false);
		buttonAddNext.setClickListener((c, mouseButton) -> {
		...
		layoutAmountAndPrice.setBackground((gui, mc, x, y, width, height, mouseX, mouseY, windowActive) -> {
			...
			Gui.drawRect(x + offsetX + 1, y + offsetY + 1, x + offsetX + 37, y + offsetY + 37,
		...
		buttonAmountAndPriceBack.setClickListener((c, mouseButton) -> setCurrentLayout(layoutSelectItem));
		...
		buttonAmountAndPriceCancel.setClickListener((c, mouseButton) -> restoreDefaultLayout());
		...
		buttonAmountAndPriceNext.setClickListener((c, mouseButton) -> setCurrentLayout(layoutDuration));
		...
		buttonDurationAdd.setClickListener((c, mouseButton) -> {
			...
			dialog.setPositiveListener((c1, mouseButton1) -> {
				int ticks = (int) TimeUtil.getRealTimeToTicks(selectorHours.getNumber(), selectorMinutes.getNumber(),
						selectorSeconds.getNumber());
				TaskAddAuction task = new TaskAddAuction(inventory.getSelectedSlotIndex(), selectorAmount.getNumber(),
						selectorPrice.getNumber(), ticks);
				task.setCallback((nbt, success) -> {
					if (success) {
