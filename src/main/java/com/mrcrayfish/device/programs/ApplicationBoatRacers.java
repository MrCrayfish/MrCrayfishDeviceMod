...
public class ApplicationBoatRacers extends Application {
	...
	public void init() {
		...
		btnNextLayer.setClickListener((c, mouseButton) -> {
			game.nextLayer();
		...
		btnPrevLayer.setClickListener((c, mouseButton) -> {
			game.prevLayer();
			labelLayer.setText(Integer.toString(game.getCurrentLayer().layer + 1));
		});
		...
		checkBoxBackground
				.setClickListener((c, mouseButton) -> game.setRenderBackground(checkBoxBackground.isSelected()));
		...
		checkBoxForeground
				.setClickListener((c, mouseButton) -> game.setRenderForeground(checkBoxForeground.isSelected()));
		...
		checkBoxPlayer.setClickListener((c, mouseButton) -> game.setRenderPlayer(checkBoxPlayer.isSelected()));
