	...
	public void update() {
		...
		if ((state == IDLE) && (remainingPrintTime == 0) && (currentPrint != null)) {
			if (!world.isRemote) {
				...
				double[] fixedPosition = CollisionHelper.fixRotation(state.getValue(BlockHorizontal.FACING), 0.15, 0.5,
						0.15, 0.5);
				EntityItem entity = new EntityItem(world, pos.getX() + fixedPosition[0], pos.getY() + 0.0625,
						pos.getZ() + fixedPosition[1], IPrint.generateItem(currentPrint));
				entity.motionX = 0;
				entity.motionY = 0;
				entity.motionZ = 0;
