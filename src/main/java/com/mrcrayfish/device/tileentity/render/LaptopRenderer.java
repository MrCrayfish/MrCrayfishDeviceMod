...
public class LaptopRenderer extends TileEntitySpecialRenderer<TileEntityLaptop> {
	...
	public void render(TileEntityLaptop te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		IBlockState state = DeviceBlocks.LAPTOP.getDefaultState().withProperty(BlockLaptop.TYPE,
				BlockLaptop.Type.SCREEN);
		...
		GlStateManager.pushMatrix();
		{
			...
			if (te.isExternalDriveAttached()) {
				GlStateManager.pushMatrix();
				{
					...
					Minecraft.getMinecraft().getRenderManager().renderEntity(entityItem, 0.0D, 0.0D, 0.0D, 0.0F, 0.0F,
							false);
			...
			GlStateManager.pushMatrix();
			{
				...
				blockrendererdispatcher.getBlockModelRenderer().renderModel(getWorld(), ibakedmodel, state, pos, buffer,
						false);
