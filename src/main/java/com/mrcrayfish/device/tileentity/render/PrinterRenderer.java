		...
		GlStateManager.pushMatrix();
		{
			...
			if (te.hasPaper()) {
				GlStateManager.pushMatrix();
				{
					...
					GlStateManager.rotate(state.getValue(BlockHorizontal.FACING).getHorizontalIndex() * -90F, 0, 1, 0);
			...
			GlStateManager.pushMatrix();
			{
				if (te.isLoading()) {
					...
					GlStateManager.rotate(state1.getValue(BlockHorizontal.FACING).getHorizontalIndex() * -90F, 0, 1, 0);
					GlStateManager.rotate(22.5F, 1, 0, 0);
					...
					double progress = -0.35
							+ (0.50 * ((double) (te.getRemainingPrintTime() - 20) / te.getTotalPrintTime()));
			...
			GlStateManager.pushMatrix();
			{
				...
				GlStateManager.rotate(22.5F, 1, 0, 0);
				Minecraft.getMinecraft().fontRenderer.drawString(Integer.toString(te.getPaperCount()), 0, 0,
						Color.WHITE.getRGB());
				GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
