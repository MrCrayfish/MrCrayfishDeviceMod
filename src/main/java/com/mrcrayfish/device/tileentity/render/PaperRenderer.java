...
public class PaperRenderer extends TileEntitySpecialRenderer<TileEntityPaper> {
	...
	public void render(TileEntityPaper te, double x, double y, double z, float partialTicks, int destroyStage,
		...
		GlStateManager.pushMatrix();
		{
			...
			if (state.getBlock() != DeviceBlocks.PAPER) {
				return;
			}
			GlStateManager.rotate((state.getValue(BlockHorizontal.FACING).getHorizontalIndex() * -90F) + 180F, 0, 1, 0);
			...
			if (print != null) {
				...
				if (data.hasKey("pixels", Constants.NBT.TAG_INT_ARRAY)
						&& data.hasKey("resolution", Constants.NBT.TAG_INT)) {
	...
	private static void drawCuboid(double x, double y, double z, double width, double height, double depth) {
		...
		drawQuad((x + width + 1) - (width + width), y, z + depth, (x + width + 1) - (width + width), y + height, z,
				EnumFacing.WEST);
		...
		for (int i = 0; i < resolution; i++) {
			for (int j = 0; j < resolution; j++) {
				...
				if (a < 1.0F) {
					...
					float r = ((pixels[j + (i * resolution)] >> 16) & 255) / 255.0F;
					float g = ((pixels[j + (i * resolution)] >> 8) & 255) / 255.0F;
					float b = (pixels[j + (i * resolution)] & 255) / 255.0F;
					GlStateManager.color(r, g, b, a);
				}
				drawCuboid((j * scale) - ((resolution - 1) * scale), (-i * scale) + ((resolution - 1) * scale), -1,
						scale, scale, 1);
			}
		}
	}
