...
public class Folder extends File {
	...
	/**
	 * Adds a file to the folder. The folder must be in the file system before you
	 * can add files to it. If the file with the same name exists, it will not
	 * overridden. This method does not verify if the file was added successfully.
	 * See {@link #add(File, Callback)} to determine if it was successful or not.
	 *
	 * @param file
	 *            the file to add
	 */
	 ...
	 * Adds a file to the folder. The folder must be in the file system before you
	 * can add files to it. If the file with the same name exists, it will not
	 * overridden. This method allows the specification of a {@link Callback}, and
	 * will return a {@link com.mrcrayfish.device.core.io.FileSystem.Response}
	 * indicating if the file was successfully added to the folder or an error
	 * occurred.
	 *
	 * @param file
	 *            the file to add
	...
	/**
	 * Adds a file to the folder. The folder must be in the file system before you
	 * can add files to it. If the file with the same name exists, it can be
	 * overridden by passing true to the override parameter. This method also allows
	 * the specification of a {@link Callback}, and will return a
	 * {@link com.mrcrayfish.device.core.io.FileSystem.Response} indicating if the
	 * file was successfully added to the folder or an error occurred.
	 *
	 * @param file
	 *            the file to add
	 * @param override
	 *            if should override existing file
	 * @param callback
	 *            the response callback
	 */
	public void add(File file, boolean override, @Nullable Callback<FileSystem.Response> callback) {
		...
		if (!FileSystem.PATTERN_FILE_NAME.matcher(file.name).matches()) {
			if (callback != null) {
				callback.execute(FileSystem.createResponse(FileSystem.Status.FILE_INVALID_NAME, "Invalid file name"),
						true);
			}
			return;
		}
		...
		if (hasFile(file.name)) {
			if (!override) {
				if (callback != null) {
					callback.execute(FileSystem.createResponse(FileSystem.Status.FILE_EXISTS,
							"A file with that name already exists"), true);
				}
				return;
			} else if (getFile(file.name).isProtected()) {
				if (callback != null) {
					callback.execute(FileSystem.createResponse(FileSystem.Status.FILE_IS_PROTECTED,
							"Unable to override protected files"), true);
				}
				return;
			}
		}
	...
	/**
	 * Deletes the specified file name from the folder. The folder must be in the
	 * file system before you can delete files from it. If the file is not found, it
	 * will just fail silently. This method does not return a response if the file
	 * was deleted successfully. See {@link #delete(String, Callback)} (File,
	 * Callback)} to determine if it was successful or not.
	 *
	 * @param name
	 *            the file name
	 */
	...
	/**
	 * Deletes the specified file name from the folder. The folder must be in the
	 * file system before you can delete files from it. This method also allows the
	 * specification of a {@link Callback} , and will return a
	 * {@link com.mrcrayfish.device.core.io.FileSystem.Response} indicating if the
	 * file was successfully deleted from the folder or an error occurred.
	 *
	 * @param name
	 * @param callback
	 */
	...
	/**
	 * Delete the specified file from the folder. The folder must be in the file
	 * system before you can delete files from it. If the file is not in this
	 * folder, it will just fail silently. This method does not return a response if
	 * the file was deleted successfully. See {@link #delete(String, Callback)}
	 * (File, Callback)} to determine if it was successful or not.
	 *
	 * @param file
	...
	/**
	 * Delete the specified file from the folder. The folder must be in the file
	 * system before you can delete files from it. The file must be in this folder,
	 * otherwise it will fail. This method also allows the specification of a
	 * {@link Callback}, and will return a
	 * {@link com.mrcrayfish.device.core.io.FileSystem.Response} indicating if the
	 * file was successfully deleted from the folder or an error occurred.
	 *
	 * @param file
	...
	public void delete(File file, @Nullable Callback<FileSystem.Response> callback) {
		...
		if (!files.contains(file)) {
			if (callback != null) {
				callback.execute(FileSystem.createResponse(FileSystem.Status.FILE_INVALID,
						"The file does not exist in this folder"), false);
			}
			return;
		}
		...
		if (file.isProtected()) {
			if (callback != null) {
				callback.execute(
						FileSystem.createResponse(FileSystem.Status.FILE_IS_PROTECTED, "Cannot delete protected files"),
						false);
			}
			return;
		}
	...
	public void copyInto(File file, boolean override, boolean cut, @Nullable Callback<FileSystem.Response> callback) {
		...
		if (!file.valid || (file.drive == null)) {
			if (callback != null) {
				callback.execute(FileSystem.createResponse(FileSystem.Status.FILE_INVALID, "Source file is invalid"),
						false);
			}
			return;
		}
		...
		FileSystem.sendAction(file.drive, FileAction.Factory.makeCopyCut(file, this, false, cut),
				(response, success) -> {
	...
	/**
	 * Gets a file from this folder for the specified name. If the file is not
	...
	/**
	 * Gets a folder from this folder for the specified name. If the folder is not
	 * found, it will return null.
	 *
	 * @param name
	...
	public Folder getFolder(String name) {
		return (Folder) files.stream().filter(file -> file.isFolder() && file.name.equalsIgnoreCase(name)).findFirst()
				.orElse(null);
	}
	...
	/**
	 * Allows you to search this folder for files using a specified predicate. This
	 * only searches the folder itself and does not include any sub-folders. Once
	 * found, it will a list of all the files that matched the predicate.
	 *
	 * @param conditions
	 *            the conditions of the file
	...
	/**
	 * Sets the data for this file. This does not work on folders and will fail
	 * silently.
	 * 
	 * @param data
	 *            the data to set
	...
	/**
	 * Sets the data for this file. This does not work on folders and will fail
	 * silently. A callback can be specified but will be a guaranteed fail for
	 * folders.
	 *
	 * @param data
	 *            the data to set
	...
	public void setData(@Nonnull NBTTagCompound data, Callback<FileSystem.Response> callback) {
		if (callback != null) {
			callback.execute(FileSystem.createResponse(FileSystem.Status.FAILED, "Can not set data of a folder"),
					false);
		}
	}
	...
	/**
	 * Converts this folder into a tag compound. Due to how the file system works,
	 * this tag does not include the name of the folder and will have to be set
	 * manually for any storage.
	 *
	 ...
	 * @param folderTag
	 *            the tag compound from {@link #toTag()}
	...
	/**
	 * Returns a copy of this folder. The copied folder is considered invalid and
	...
	/**
	 * Returns a copy of this folder with a different name. The copied folder is
	 * considered invalid and changes to it can not be made until it is added into
	 * the file system.
	 *
	 * @param newName
