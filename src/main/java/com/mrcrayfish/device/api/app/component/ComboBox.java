...
public abstract class ComboBox<T> extends Component {
	...
	public void init(Layout layout) {
		this.layout.setBackground((gui, mc, x, y, width, height, mouseX, mouseY, windowActive) -> Gui.drawRect(x, y,
				x + width, y + height, Color.GRAY.getRGB()));
	}
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive,
		...
		if (visible) {
			...
			RenderUtil.drawRectWithTexture(((xPosition + height) - 2) + xOffset, yPosition, 99 + (i * 5), 12, 2, 2, 2,
					2);
			RenderUtil.drawRectWithTexture(((xPosition + height) - 2) + xOffset, (yPosition + height) - 2, 99 + (i * 5),
					15, 2, 2, 2, 2);
			...
			RenderUtil.drawRectWithTexture(((xPosition + height) - 2) + xOffset, yPosition + 2, 99 + (i * 5), 14, 2,
					height - 4, 2, 1);
			RenderUtil.drawRectWithTexture(xPosition + 2 + xOffset, (yPosition + height) - 2, 98 + (i * 5), 15,
					height - 4, 2, 1, 2);
			RenderUtil.drawRectWithTexture(xPosition + xOffset, yPosition + 2, 96 + (i * 5), 14, 2, height - 4, 2, 1);

			/* Center */
			RenderUtil.drawRectWithTexture(xPosition + 2 + xOffset, yPosition + 2, 98 + (i * 5), 14, height - 4,
					height - 4, 1, 1);
			...
			drawRect(xPosition + 1, yPosition + 1, xPosition + xOffset, (yPosition + height) - 1,
	...
	protected boolean isInside(int mouseX, int mouseY) {
		return (mouseX >= xPosition) && (mouseY >= yPosition) && (mouseX < (xPosition + this.width))
				&& (mouseY < (yPosition + this.height));
	}
	...
	public static class List<T> extends ComboBox<T> {
		...
		public List(int left, int top, T[] items) {
			...
			layout = new Layout(width, getListHeight(list));
			this.setItems(items);
		}
		...
		public List(int left, int top, int width, T[] items) {
			...
			layout = new Layout(width + 50, getListHeight(list));
			this.setItems(items);
		}
		...
		public List(int left, int top, int comboBoxWidth, int listWidth, T[] items) {
			...
			layout = new Layout(listWidth, getListHeight(list));
			this.setItems(items);
		}
	...
	public static class Custom<T> extends ComboBox<T> {
		public Custom(int left, int top, int width, int contextWidth, int contextHeight) {
			super(left, top);
			this.width = width;
			layout = new Layout.Context(contextWidth, contextHeight);
		}
