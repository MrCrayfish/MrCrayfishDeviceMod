...
public class TextArea extends Component {
	...
	private static final String[] DELIMITERS = { "(\\s|$)(?=(([^\"]*\"){2})*[^\"]*$)", "[\\p{Punct}&&[^@\"]]",
			"\\p{Digit}+" };
	...
	public TextArea(int left, int top, int width, int height) {
		super(left, top);
		fontRenderer = Laptop.fontRenderer;
		this.width = width;
		this.height = height;
		visibleLines = (int) Math.floor(((height - (padding * 2)) + 1) / fontRenderer.FONT_HEIGHT);
		lines.add("");
	}
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive,
		...
		if (visible) {
			...
			for (int i = 0; (i < visibleLines) && ((i + verticalScroll) < lines.size()); i++) {
				...
				int scrollX = MathHelper.clamp(horizontalScroll + (int) (horizontalOffset * pixelsPerUnit), 0,
						Math.max(0, maxLineWidth - (width - (padding * 2))));
				...
				if (highlight != null) {
					...
					fontRenderer.drawString(builder.toString(), (x + padding) - scrollX,
			...
			if (editable && isFocused) {
				...
				int scroll = MathHelper.clamp(verticalScroll + (verticalOffset * (int) linesPerUnit), 0,
						Math.max(0, lines.size() - visibleLines));
				if (isFocused && (cursorY >= scroll) && (cursorY < (scroll + visibleLines))) {
					if (((cursorTick / 10) % 2) == 0) {
						...
						int posX = (x + padding + stringWidth)
								- MathHelper.clamp(horizontalScroll + (int) (horizontalOffset * pixelsPerUnit), 0,
										Math.max(0, maxLineWidth - visibleWidth));
						int posY = y + padding + ((cursorY - scroll) * fontRenderer.FONT_HEIGHT);
						Gui.drawRect(posX, posY - 1, posX + 1, (posY + fontRenderer.FONT_HEIGHT) - 1,
								Color.WHITE.getRGB());
					}
				}
			}
			...
			if (scrollBarVisible) {
				if (lines.size() > visibleLines) {
					...
					int scrollBarHeight = Math.max(20,
							(int) (((float) visibleLines / (float) lines.size()) * visibleScrollBarHeight));
					float scrollPercentage = MathHelper.clamp(
							(verticalScroll + verticalOffset) / (float) (lines.size() - visibleLines), 0.0F, 1.0F);
				...
				if (!wrapText && (maxLineWidth >= (width - (padding * 2)))) {
					...
					float scrollPercentage = (float) (horizontalScroll + 1)
							/ (float) ((maxLineWidth - visibleWidth) + 1);
					int scrollBarWidth = Math.max(20,
							(int) (((float) visibleWidth / (float) maxLineWidth) * visibleScrollBarWidth));
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton) {
		...
		isFocused = GuiHelper.isMouseInside(mouseX, mouseY, xPosition, yPosition, xPosition + width,
				yPosition + height);
		...
		if (GuiHelper.isMouseWithin(mouseX, mouseY, xPosition + padding, yPosition + padding, width - (padding * 2),
				height - (padding * 2))) {
	...
	protected void handleMouseDrag(int mouseX, int mouseY, int mouseButton) {
		if (scrollBar != null) {
			...
			case VERTICAL:
				...
				int scrollBarHeight = Math.max(20,
						(int) (((float) visibleLines / (float) lines.size()) * visibleScrollBarHeight));
				float spacing = (float) (visibleScrollBarHeight - scrollBarHeight)
						/ (float) (lines.size() - visibleLines);
	...
	protected void handleMouseRelease(int mouseX, int mouseY, int mouseButton) {
		if (scrollBar != null) {
			...
			case HORIZONTAL: {
				...
				horizontalScroll = MathHelper.clamp(horizontalScroll + (int) (horizontalOffset * scrollPercentage), 0,
						maxLineWidth - (width - (padding * 2)));
				break;
			}
			case VERTICAL: {
				float scrollPercentage = MathHelper
						.clamp((verticalScroll + verticalOffset) / (float) (lines.size() - visibleLines), 0.0F, 1.0F);
	...
	public void handleKeyTyped(char character, int code) {
		if (!visible || !enabled || !isFocused || !editable) {
			return;
		}
	...
	private ScrollBar isMouseInsideScrollBar(int mouseX, int mouseY) {
		...
		if (lines.size() > visibleLines) {
			...
			int posY = yPosition + 2
					+ MathHelper.clamp(relativeScrollY + verticalOffset, 0, visibleScrollBarHeight - scrollBarHeight);
	...
	private void moveCursorRight(int amount) {
		...
		if (((cursorY == (lines.size() - 1)) && (cursorX == activeLine.length()))
				|| ((cursorX > 0) && (activeLine.charAt(cursorX - 1) == '\n'))) {
			return;
		}
	...
	private void updateText() {
		...
		if (wrapText) {
			...
			List<String> split = fontRenderer.listFormattedStringToWidth(lines.get(lines.size() - 1),
					width - (padding * 2));
			...
			List<String> activeLine = fontRenderer.listFormattedStringToWidth(lines.get(cursorY),
					width - (padding * 2));
			int totalLength = 0;
	...
	/**
	 * Sets the placeholder for the text area. This is the text that is shown if no
	 * text is present and gives a hint to the user what this text area is for.
	 *
	 * @param placeholder
	...
	/**
	 * Sets the highlighting for the text area. This is used, for instance, where
	 * you want particular keywords to be a different colour from the rest.
	 *
	 * @param highlight
	 *            the highlight to colour the text
	 */
	 ...
	 *
	 * @param isFocused
	 *            whether the text area should be focused
	 */
	...
	public void setPadding(int padding) {
		...
		visibleLines = (int) Math.floor((height - (padding * 2)) / fontRenderer.FONT_HEIGHT);
	}
	...
	/**
	 * Sets the maximum amount of lines that the text area can have. If the maximum
	 * lines is set to zero or below, the text area will ignore the max line
	 * property. It's suggested that this method should not be called in any other
	 * place besides the initialization of the component.
	 *
	 * @param maxLines
	 *            the maximum amount of lines for the text area
	 */
