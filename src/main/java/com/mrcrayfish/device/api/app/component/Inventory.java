 ...
 * is in quotes is because it's client side only. If you want to process
 * anything, you'll have to sendTask the selected item slot to the server and
 * process it there. You can use a {@link Task} to perform this.
 *
 * @author MrCrayfish
 */
public class Inventory extends Component {
	protected static final ResourceLocation CHEST_GUI_TEXTURE = new ResourceLocation(
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive,
		...
		if (visible) {
			...
			for (int i = 9; i < (inventory.getSizeInventory() - 4); i++) {
				...
				if (selected == i) {
					Gui.drawRect(xPosition + offsetX, yPosition + offsetY, xPosition + offsetX + 18,
							yPosition + offsetY + 18, selectedColour);
				}

				if (GuiHelper.isMouseInside(mouseX, mouseY, xPosition + offsetX, yPosition + offsetY,
						xPosition + offsetX + 17, yPosition + offsetY + 17)) {
					Gui.drawRect(xPosition + offsetX, yPosition + offsetY, xPosition + offsetX + 18,
							yPosition + offsetY + 18, hoverColour);
				}
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 9; j++) {
					...
					if (GuiHelper.isMouseInside(mouseX, mouseY, x, y, x + 18, y + 18)) {
						...
						if (!stack.isEmpty()) {
							laptop.drawHoveringText(Arrays.asList(new String[] { stack.getDisplayName() }), mouseX,
									mouseY);
						}
						return;
					}
