...
public class ItemList<E> extends Component implements Iterable<E> {
	...
	/**
	 * Default constructor for the item list. Should be noted that the height is
	 * determined by how many visible items there are.
	 *
	 * @param left
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive,
		...
		if (visible) {
			...
			/* Fill */
			Gui.drawRect(xPosition + 1, yPosition + 1, (xPosition + width) - 1, yPosition + (size * height) + size,
			...
			for (int i = 0; (i < (size - 1)) && (i < items.size()); i++) {
				...
				if (item != null) {
					if (renderer != null) {
						renderer.render(item, this, mc, xPosition + 1, yPosition + (i * (renderer.getHeight())) + 1 + i,
								width - 2, renderer.getHeight(), (i + offset) == selected);
						drawHorizontalLine(xPosition + 1, (xPosition + width) - 1,
								yPosition + (i * height) + i + height + 1, borderColour);
					} else {
						drawRect(xPosition + 1, yPosition + (i * 14) + 1, (xPosition + width) - 1,
								yPosition + 13 + (i * 14) + 1,
								(i + offset) != selected ? backgroundColour : Color.DARK_GRAY.getRGB());
						drawString(mc.fontRenderer, item.toString(), xPosition + 3, yPosition + 3 + (i * 14),
								textColour);
						drawHorizontalLine(xPosition + 1, (xPosition + width) - 2,
								yPosition + (i * height) + i + height + 1, Color.DARK_GRAY.getRGB());
					}
				}
			}
			...
			if (items.size() > visibleItems) {
				drawRect(xPosition + width, yPosition, xPosition + width + 10, yPosition + (size * height) + size,
						Color.DARK_GRAY.getRGB());
				drawVerticalLine(xPosition + width + 10, yPosition + 11, (yPosition + (size * height) + size) - 11,
						borderColour);
			}
		}
	}
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton) {
		...
		int size = getSize();
		if (GuiHelper.isMouseInside(mouseX, mouseY, xPosition, yPosition, xPosition + width,
				yPosition + (size * height) + size)) {
			for (int i = 0; (i < size) && (i < items.size()); i++) {
				if (GuiHelper.isMouseInside(mouseX, mouseY, xPosition + 1, yPosition + (i * height) + i,
						(xPosition + width) - 1, yPosition + (i * height) + i + height)) {
					if (mouseButton == 0) {
