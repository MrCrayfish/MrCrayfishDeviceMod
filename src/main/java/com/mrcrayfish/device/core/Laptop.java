...
public class Laptop extends GuiScreen implements System {
	...
	private static final ResourceLocation LAPTOP_GUI = new ResourceLocation(Reference.MOD_ID,
			"textures/gui/laptop.png");

	public static final ResourceLocation ICON_TEXTURES = new ResourceLocation(Reference.MOD_ID,
			"textures/atlas/app_icons.png");
	...
	public Laptop(TileEntityLaptop laptop) {
		...
		settings = Settings.fromTag(systemData.getCompoundTag("Settings"));
		bar = new TaskBar(APPLICATIONS);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawTexturedModalRect((posX + DEVICE_WIDTH) - BORDER, (posY + DEVICE_HEIGHT) - BORDER, 11, 11, BORDER,
				BORDER); // BOTTOM-RIGHT
		...
		RenderUtil.drawRectWithTexture((posX + DEVICE_WIDTH) - BORDER, posY + BORDER, 11, 10, BORDER, SCREEN_HEIGHT,
				BORDER, 1); // RIGHT
		RenderUtil.drawRectWithTexture(posX + BORDER, (posY + DEVICE_HEIGHT) - BORDER, 10, 11, SCREEN_WIDTH, BORDER, 1,
				BORDER); // BOTTOM
		...
		if (!MrCrayfishDeviceMod.DEVELOPER_MODE) {
			drawString(fontRenderer, "Alpha v" + Reference.VERSION, posX + BORDER + 5, posY + BORDER + 5,
					Color.WHITE.getRGB());
		} else {
			drawString(fontRenderer, "Developer Version - " + Reference.VERSION, posX + BORDER + 5, posY + BORDER + 5,
					Color.WHITE.getRGB());
		}

		boolean insideContext = false;
		if (context != null) {
			insideContext = GuiHelper.isMouseInside(mouseX, mouseY, context.xPosition, context.yPosition,
					context.xPosition + context.width, context.yPosition + context.height);
		}
		...
		for (int i = windows.length - 1; i >= 0; i--) {
			...
			if (window != null) {
				window.render(this, mc, posX + BORDER, posY + BORDER, mouseX, mouseY, (i == 0) && !insideContext,
						partialTicks);
			}
		}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (context != null) {
			...
			if (GuiHelper.isMouseInside(mouseX, mouseY, dropdownX, dropdownY, dropdownX + context.width,
					dropdownY + context.height)) {
		...
		bar.handleClick(this, posX, (posY + SCREEN_HEIGHT) - TaskBar.BAR_HEIGHT, mouseX, mouseY, mouseButton);
			...
			if (window != null) {
				if (window.content.getInfo().equals(app.getInfo())) {
					if (app.isDirty()) {
						...
						TaskManager.sendTask(new TaskUpdateApplicationData(pos.getX(), pos.getY(), pos.getZ(),
								app.getInfo().getFormattedId(), container));
					}
	...
	private boolean isMouseWithinWindowBar(int mouseX, int mouseY, Window<?> window) {
		...
		int posX = (width - SCREEN_WIDTH) / 2;
		int posY = (height - SCREEN_HEIGHT) / 2;
		return GuiHelper.isMouseInside(mouseX, mouseY, posX + window.offsetX + 1, posY + window.offsetY + 1,
				(posX + window.offsetX + window.width) - 13, posY + window.offsetY + 11);
	}
	...
	private boolean isMouseWithinWindow(int mouseX, int mouseY, Window<?> window) {
		...
		int posX = (width - SCREEN_WIDTH) / 2;
		int posY = (height - SCREEN_HEIGHT) / 2;
		return GuiHelper.isMouseInside(mouseX, mouseY, posX + window.offsetX, posY + window.offsetY,
				posX + window.offsetX + window.width, posY + window.offsetY + window.height);
	}

	public boolean isMouseWithinApp(int mouseX, int mouseY, Window<?> window) {
		int posX = (width - SCREEN_WIDTH) / 2;
		int posY = (height - SCREEN_HEIGHT) / 2;
		return GuiHelper.isMouseInside(mouseX, mouseY, posX + window.offsetX + 1, posY + window.offsetY + 13,
				(posX + window.offsetX + window.width) - 1, (posY + window.offsetY + window.height) - 1);
	}
	...
	public Application getApplication(String appId) {
		return APPLICATIONS.stream().filter(app -> app.getInfo().getFormattedId().equals(appId)).findFirst()
				.orElse(null);
	}
