...
public class Window<T extends Wrappable> {
	...
	public void render(Laptop gui, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean active,
		...
		if (mc.fontRenderer.getStringWidth(windowTitle) > (width - 2 - 13 - 3)) // window width, border, close button,
																				// padding, padding
		{
		...
		/* Render content */
		content.render(gui, mc, x + offsetX + 1, y + offsetY + 13, mouseX, mouseY, active && (dialogWindow == null),
				partialTicks);
	...
	public void openDialog(Dialog dialog) {
		if (dialogWindow != null) {
			...
			dialogWindow = new Window<Dialog>(dialog, null);
