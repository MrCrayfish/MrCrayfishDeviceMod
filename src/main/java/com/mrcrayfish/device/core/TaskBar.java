...
public class TaskBar {
	...
	public void init(int posX, int posY) {
		...
		btnRight.setClickListener((c, mouseButton) -> {
	...
	public void render(Laptop gui, Minecraft mc, int x, int y, int mouseX, int mouseY, float partialTicks) {
		...
		mc.fontRenderer.drawString(timeToString(mc.player.world.getWorldTime()), x + 334, y + 5, Color.WHITE.getRGB(),
				true);
		...
		if (isMouseInside(mouseX, mouseY, x + 18, y + 1, x + 236, y + 16)) {
			...
			if ((appIndex < (offset + APPS_DISPLAYED)) && (appIndex < applications.size())) {
				...
				gui.drawHoveringText(Collections.singletonList(applications.get(appIndex).getInfo().getName()), mouseX,
						mouseY);
			}
		}
