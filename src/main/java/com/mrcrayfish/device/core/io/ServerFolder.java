...
public class ServerFolder extends ServerFile {
	...
	public ServerFolder getFolder(String name) {
		return (ServerFolder) files.stream().filter(file -> file.isFolder() && file.name.equalsIgnoreCase(name))
	...
	/*
	 * public void print(int startingDepth) { String indent = ""; for(int i = 0; i <
	 * startingDepth; i++) { indent += "  "; }
	 * MrCrayfishDeviceMod.getLogger().info(indent + "⌊ " + name); for(ServerFile
	 * file : files) { if(file.isFolder()) { ((ServerFolder)
	 * file).print(startingDepth + 1); } else {
