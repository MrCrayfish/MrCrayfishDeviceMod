...
public abstract class AbstractDrive {
	...
	AbstractDrive(String name) {
		...
		root = createProtectedFolder("Root");
	}
		...
		if (folder != null) {
			...
			case NEW:
				if (data.hasKey("files", Constants.NBT.TAG_COMPOUND)) {
					return folder.add(ServerFolder.fromTag(actionData.getString("file_name"), data),
							actionData.getBoolean("override"));
				}
				return folder.add(ServerFile.fromTag(actionData.getString("file_name"), data),
						data.getBoolean("override"));
			case DELETE:
			...
			case RENAME:
				...
				return FileSystem.createResponse(FileSystem.Status.FILE_INVALID,
						"File not found on server. Please refresh!");
			case DATA:
					...
					if (drive != null) {
						...
						if (destination != null) {
							...
							while (true) {
								...
								if (temp == file) {
									return FileSystem.createResponse(FileSystem.Status.FAILED,
											"Destination folder can't be a subfolder");
								}
								temp = temp.getParent();
							}

							FileSystem.Response response = destination.add(file.copy(),
									actionData.getBoolean("override"));
						...
						return FileSystem.createResponse(FileSystem.Status.FILE_INVALID,
								"Destination folder not found on server. Please refresh!");
					}
					return FileSystem.createResponse(FileSystem.Status.DRIVE_UNAVAILABLE,
							"Drive unavailable. Please refresh!");
				}
				return FileSystem.createResponse(FileSystem.Status.FILE_INVALID,
						"File not found on server. Please refresh!");
			}
	...
	private static ServerFolder createProtectedFolder(String name) {
		try {
			Constructor<ServerFolder> constructor = ServerFolder.class.getDeclaredConstructor(String.class,
					boolean.class);
